...
int bn_init_int(bn *t, int init_int) {
    ...
    if (init_int > 0) t->sign = 1;
        ...
        t->sign = -1;
        init_int *= -1;
    } /// если отрицательное то привращаем в положительное
...
bn *bn_sub_sign(bn const *left, bn const *right) { ///Вычитание BN одного знака left - right
    ...
    for (int i = 0; i < left->bodysize; i++) {
        res->bodysize++;
        if (i <
            right->bodysize) {                                     ///если меньше меньшего, то отнимаем цифры обоих чисел
...
bn *bn_mul(bn const *left, bn const *right) {
    ...
    bn *res = bn_new();
    bn *right_cp_base = bn_init(
            right); ///Здесь хранится копия правого числа, которая умножается на 10 каждый раз кроме 1 итерации
    res->sign = left->sign * right->sign;
    for (int i = 0; i < left->bodysize; i++) {
        if (i > 0) bn_mul_small(right_cp_base, base);
        bn *right_cp = bn_init(
                right_cp_base); /// Временная копия, которая нужна для операций чтобы не менять исходное число
...
bn *bn_div(bn const *left, bn const *right) {
    ...
    for (int i = left->bodysize - 1; i >= 0; i--) {
        ...
        int flag = -1;
        int dig_counter = 0;
        while (bn_cmp_abs(current, right) ==
               -1) {                         ///записываем в current пока оно меньше чем right
            k++;
            if (i - k < 0) {
                if (flag !=
                    -1)                                   /// если добавляли нули в ответ, то bodysize на 1 больше чем нужно
                    res->bodysize--;
                break;
            }
            if (current->bodysize >= 2 || ((k >= 1 || res->bodysize > 0) && current->body[current->bodysize - 1] !=
                                                                            0)) {///из-за обратного хранения сдвигаем все цифры вправо и записываем в начало
                dig_counter++;
            ...
            if ((res->bodysize > 0 && current->body[current->bodysize - 1] == 0) || (res->bodysize > 0 && dig_counter >=
                                                                                                          2)) { ///если первая цифра в current постоянно 0, то дописываем нули в ответ
...
bn *bn_mod(bn const *left, bn const *right) {
    ...
    if (bn_cmp_abs(left, right) == -1) {
        ...
        tmp->sign = right->sign;
        return tmp;
    }  ///если модуль делителя больше делимого то остаток само число
    bn *res = bn_new();
    res->body = realloc(res->body, left->bodysize * sizeof(ll));
    res->bodysize = 0;
    bn *current = bn_new();
...
int bn_root_to(bn *t, int reciprocal) {
    ...
    while (1) {
        ...
        if (p == 0 || (p > 0 &&
                       flag)) {                          ///если числа равны, либо предыдущее было меньше, а сейчас стало увеличиваться, то выход
            t->sign = sign;
...
const char *bn_to_string(bn const *t, int radix) {
    ...
    if (t->sign ==
        -1) {                                                           ///если отрицательное то добавляем - в конец строки
        if (i == reall_size)
            result_string = (char *) realloc(result_string, reall_size *= 2); ///если память переполнилась
        result_string[i] = '-';
        i++;
    }
